...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/api/state'&&req.method==='GET'){const visits=db.prepare('SELECT date,child FROM visits ORDER BY date').all();return json(res,200,{today:today(),authenticated,children:children.map(({file,...c})=>c),visits,forecasts:children.map(c=>({child:c.id,...predict(visits.filter(v=>v.child===c.id).map(v=>v.date),today())}))});}
